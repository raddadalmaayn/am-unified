...
/*
 * E1 / D3 (2026-08-11). The steady-state window has ONE implementation, in
 * steady.js, and this file imports it rather than carrying a second copy.
 * `steady_state` below is that authoritative figure.
 *
 * The superseded definition is retained, computed alongside and reported as
 * `steady_state_legacy_superseded`, for one reason only: the Phase 4
 * three-column comparison table needs the old figure as its middle column.
 * It is NOT the source of any paper number.
 */
const { computeSteady } = require('./steady.js');
...
function analyzeRun(file) {
  ...
  // ── SUPERSEDED (B2 drain exclusion) — retained for the Phase 4 comparison ──
  // This opened the window at the first non-warm-up record rather than at the
  // moment the last warm-up transaction resolved, so it opened too early. E1
  // replaced it; see steady.js. Computed here ONLY to populate the middle
  // column of the Phase 4 three-way table. Never a reported result.
  const slots = new Set(recs.map(r => r.worker_slot)).size;

  const steadySorted = steady
    .filter(r => r.t_committed_ns)
    .sort((a, b) => {
  ...
  let legacy_tps = null, legacy_window_ms = null;
  if (retained.length >= 2) {
    const first = BigInt(retained[0].t_committed_ns);
    const last  = BigInt(retained[retained.length - 1].t_committed_ns);
    legacy_window_ms = Number(last - first) / 1e6;
    // n-1 completions occur strictly inside the window between first and last.
    legacy_tps = legacy_window_ms > 0 ? ((retained.length - 1) / (legacy_window_ms / 1000)) : null;
  }

  // ── AUTHORITATIVE (E1): the single implementation, from steady.js ──
  const e1 = computeSteady(file);
  ...
  const stats = {
    ...
    // AUTHORITATIVE — E1, computed by steady.js. Use this.
    steady_state: e1 ? {
      window_ms: e1.duration_s == null ? null : e1.duration_s * 1000,
      tps: e1.steady_tps, count: e1.count, W: e1.W, total: e1.total,
      t_start_ns: e1.t_start_ns, t_end_ns: e1.t_end_ns,
      definition: 'E1 (steady.js): interval during which exactly W transactions were in flight throughout',
      note: e1.note,
    } : { window_ms: null, tps: null, definition: 'E1 (steady.js)', note: 'not computable from this file' },
    // SUPERSEDED — retained only for the Phase 4 three-column comparison.
    steady_state_legacy_superseded: {
      window_ms: legacy_window_ms, tps: legacy_tps,
      retained_count: retained.length, dropped_drain_count: dropCount,
      drain_rule: 'trailing `observed_worker_slots` completions excluded',
      status: 'SUPERSEDED by E1 — not a reported result, not a source of any paper number',
    },
    suspect_low_steady_count: steady.length < 50,
  };
  return stats;
}
...
function aggregate(runStats) {
  ...
  for (const [cond, runs] of Object.entries(byCond)) {
    ...
    const tpssLegacy = runs.map(r => r.steady_state_legacy_superseded.tps).filter(v => v != null);
    ...
    out[cond] = {
      ...
      per_run: runs.map(r => ({
        ...
        steady_tps_legacy_superseded: r.steady_state_legacy_superseded.tps,
        errors_by_class: r.errors_by_class,
      })),
      median_across_runs: {
        p50: median(p50s), steady_tps: median(tpss),
        steady_tps_legacy_superseded: median(tpssLegacy),
