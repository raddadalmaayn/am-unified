...
/*
 * Phase 8 (2026-08-09): network identity is overridable via environment so the
 * SAME harness drives both the four-node lab (amchannel, ~/fabric-network) and
 * the single-host two-org network (mychannel, AM/fabric-samples/test-network).
 * Forking the harness would have made the two testbeds' manifests
 * non-comparable, which is the whole point of re-running Tables 6 and 7.
 * Defaults are the lab, so every lab invocation is byte-identical to before.
 */
const channelName   = process.env.BENCH_CHANNEL   || 'amchannel';
const chaincodeName = process.env.BENCH_CC_NAME   || 'unified';
const mspId         = process.env.BENCH_MSPID     || 'Org1MSP';
const cryptoBase    = process.env.BENCH_CRYPTO_BASE ||
                      path.resolve(process.env.HOME, 'fabric-network', 'peerOrganizations', 'org1.example.com');
const peerEndpoint  = process.env.BENCH_PEER_ENDPOINT || 'localhost:7051';
...
const FABINFO = process.env.BENCH_FABINFO ||
  path.resolve(process.env.HOME, 'am-unified', 'client-tests', 'fabinfo.sh');
const CC_SHA256_EXPECTED = process.env.BENCH_CC_SHA256 ||
  '46ae8a9f2cfcac4cf967ddcd0bf47e381f3f3377b8647b52c94dd39cb40ecff5';
...
/**
 * DEFECT FIX (2026-08-09, Phase 8). This previously took keystore file [0] and
 * assumed it paired with signcert [0]. A keystore can legitimately hold more
 * than one key — re-enrolling an identity against a Fabric CA adds a second key
 * beside the original without removing it — and when [0] is the stale key every
 * transaction fails peer-side with
 *   "error validating proposal: access denied: channel [X] creator org [Y]"
 * which reads like an MSP or policy problem rather than a client key-selection
 * bug. Select the key that actually matches the signing certificate.
 */
async function newSigner(username, credentials) {
  const keyDir = path.join(cryptoBase, 'users', `${username}@org1.example.com`, 'msp', 'keystore');
  const files = (await fsp.readdir(keyDir)).sort();
  const certPub = new crypto.X509Certificate(credentials).publicKey
    .export({ type: 'spki', format: 'der' });

  const tried = [];
  for (const f of files) {
    const pem = await fsp.readFile(path.join(keyDir, f));
    try {
      ...
      tried.push(f);
    } catch (e) { tried.push(`${f} (unreadable: ${e.message.slice(0, 60)})`); }
  }
  throw new Error(`no key in ${keyDir} matches the signcert for ${username}; ` +
                  `tried ${files.length}: ${tried.join(', ')}`);
}
 ...
 */
async function newGatewayForUser(client, username, dl) {
  const identity = await newIdentity(username);
  const signer   = await newSigner(username, identity.credentials);
  return connect({
    client, identity, signer,
    evaluateOptions:     () => ({ deadline: Date.now() + dl.evaluate }),
    endorseOptions:      () => ({ deadline: Date.now() + dl.endorse }),
    submitOptions:       () => ({ deadline: Date.now() + dl.submit }),
    commitStatusOptions: () => ({ deadline: Date.now() + dl.commitStatus }),
  });
}
...
/**
 * B3: read each user's on-chain stake balance. Read-only evaluate.
 * Note recorded from source inspection: SubmitRating compares
 * Balance < MinStakeRequired and never deducts; the only debit paths are
 * InitiateDispute (DisputeCost) and the slash path. These balances therefore
 * measure that fact rather than assuming it.
 */
/**
 * DR4: state size, read via IntegrationContract:GetSupplyChainMetrics — the same
 * method used at Gate 0, so the figures are comparable across checkpoints.
 *
 * `totalAssets` is KNOWN DEFECTIVE and always reads 0: assets are stored under
 * bare asset IDs with no key prefix, so the range scan cannot match them. It is
 * recorded verbatim and excluded from `counted_keys`. No estimate is
 * substituted, and total LevelDB key count remains unobtainable read-only
 * (`ledgerutil` in this build exposes only compare/identifytxs/verify).
 */
async function queryStateKeys(integContract) {
  try {
    const raw = await integContract.evaluateTransaction('GetSupplyChainMetrics');
    const m = JSON.parse(utf8Decoder.decode(raw));
    return {
      activeActors: m.activeActors, totalRatings: m.totalRatings,
      linkedEvents: m.linkedEvents, totalDisputes: m.totalDisputes,
      totalAssets_DEFECTIVE_ALWAYS_ZERO: m.totalAssets,
      counted_keys: (m.activeActors || 0) + (m.totalRatings || 0) + (m.linkedEvents || 0),
    };
  } catch (e) { return { error: String(e.message).slice(0, 200) }; }
}

async function queryStakes(gateways, identityIds) {
  const out = {};
  for (const [name, gw] of Object.entries(gateways)) {
    try {
      const rep = gw.getNetwork(channelName).getContract(chaincodeName, REP_CC);
      const raw = await rep.evaluateTransaction('GetStake', identityIds[name]);
      const st = JSON.parse(utf8Decoder.decode(raw));
      out[name] = { balance: st.balance, locked: st.locked };
...
async function runLoad({ total, W, taskFn, writer, warmupCount, warmupMs }) {
  ...
  const worker = async (slot) => {
    while (true) {
      ...
      // B1: the time-based clause is capped at 20% of the run so that a slow
      // high-W ramp cannot mark the entire condition as warm-up.
      const isWarmup = (seq < warmupCount) || (elapsedMs < warmupMs && seq < total * 0.2);
 ...
 */
/*
 * A3 (2026-08-09): timeout raised from 180 s to 900 s, and made CLI-settable via
 * --converge-timeout. Phase 3 measured 65-78 s convergence at W=20 against
 * ~250 ms at W=1; at higher W the 180 s ceiling was a live risk of discarding
 * runs for an operational wait rather than a fault.
 *
 * A3 also reclassifies this from an operational wait to a first-class
 * measurement. `laggards_last_poll` records which peers were still behind on
 * the final non-converged poll, i.e. which peer was last to converge.
 */
let CONVERGE_TIMEOUT_MS = 900000;
let FLUSH_EVERY = 100;   // records buffered before appendFileSync; see runCondition

function fabInfoConverged(timeoutMs = CONVERGE_TIMEOUT_MS, pollMs = 1000) {
  const t0 = Date.now();
  let last = null;
  let attempts = 0;
  let laggardsPrev = null;
  while (Date.now() - t0 < timeoutMs) {
    ...
    {
      const nums = hs.filter(h => typeof h === 'number');
      if (nums.length) {
        const hmax = Math.max(...nums);
        const behind = orgs.filter(o => {
          const h = last.peers && last.peers[o] && last.peers[o].height;
          return typeof h !== 'number' || h < hmax;
        });
        if (behind.length) laggardsPrev = { peers: behind, target_height: hmax, at_poll: attempts };
      }
    }
    ...
    if (hs.every(h => typeof h === 'number') &&
      ...
      last.convergence = { converged: true, wait_ms: Date.now() - t0, polls: attempts,
                           timeout_ms: timeoutMs, poll_ms: pollMs,
                           laggards_last_poll: laggardsPrev };
      return last;
    }
...
const NODES = process.env.BENCH_NODES
  ? JSON.parse(process.env.BENCH_NODES)
  : { D1: 'D1', D2: 'D2', D3: 'D3', D4: 'D4' };

// Peer orgs whose heights and hashes must agree. Four in the lab, two on the
// single-host network. Convergence, height_delta and the agreement checks all
// derive from this list, so it is the single place the topology is expressed.
const ORGS = (process.env.BENCH_ORGS || 'org1,org2,org3,org4').split(',');
 ...
 */
function makeWorkload(condition, run_id) {
  ...
  switch (condition) {
    ...
    case 'E': // concurrent provenance, distinct keys
      return (seq) => {
        ...
        return { key: assetID, cc: 'prov', fn: 'CreateMaterialCertification',
    ...
    case 'F': // concurrent reputation, distinct actor-dimension pairs
      return (seq) => {
        ...
        return { key: `${actor}:quality`, cc: 'rep', fn: 'SubmitRating',
    ...
    case 'G': // concurrent bridge, distinct keys
      return (seq) => {
        ...
        return { key: assetID, cc: 'integ', fn: 'RecordProvenanceWithReputation',
    ...
    case 'D': // read: GetPartTrustReport
      return (seq) => ({ key: null, cc: 'integ', fn: 'GetPartTrustReport',
                         args: [], read: true });
    default:
...
async function runCondition(opts) {
  ...
  // Phase 6 sets --flush-every=1 so txs.jsonl is a live cursor the external
  // fault injector can read to fire "at approximately transaction N". At the
  // default 100 the file lags by up to 100 records, which at W=1 and ~2.3 TPS
  // is a 40-second aiming error on the injection point.
  const writer = new JsonlWriter(jsonlPath, FLUSH_EVERY);

  const wallClockStart = new Date().toISOString();
  console.log(`\n[${condition} run${run_index}] W=${W} total=${total} run_id=${run_id}`);

  // B3: rating conditions re-stake before every run and record the balance on
  // both sides of the run, so stake sufficiency is measured rather than assumed.
  const STAKE_CONDITIONS = new Set(['B', 'F', 'H']);
  let stake_before = null, stake_after = null;
  if (STAKE_CONDITIONS.has(condition)) {
    for (const [name, gw] of Object.entries(opts.gateways)) {
      try {
        await gw.getNetwork(channelName).getContract(chaincodeName, REP_CC)
          .submitTransaction('AddStake', MIN_STAKE);
      } catch (e) { console.log(`  stake top-up ${name}: ${e.message.slice(0, 80)}`); }
    }
    stake_before = await queryStakes(opts.gateways, opts.identityIds);
    console.log(`  stake before: ${JSON.stringify(stake_before)}`);
  }

  // 1h + D3: ledger and orderer state BEFORE
  const before = fabInfo();

  // DR4 (2026-08-09): record state size in every manifest, so the intra-session
  // state gradient is a per-run datum rather than a phase-level footnote.
  const state_keys_before = await queryStateKeys(opts.contracts.integ);

  const isRead = READ_CONDITIONS.has(condition);
  const workload = makeWorkload(condition, run_id);
  // B1: scale warm-up with the run, never with W alone. At W=100/200 the old
  // rule (5*W transactions or 10 s) marked every transaction as warm-up and
  // analyze.js emitted nulls for the whole condition.
  const warmupCount = Math.max(W, Math.min(5 * W, Math.floor(total * 0.1)));
  const warmupMs = 2000;

  // PROBE: worker slot i uses client (i % clientsN). With clientsN=1 every
  // worker shares one grpc.Client, i.e. one HTTP/2 connection.
  const pools = opts.pools;
  const taskFn = async (seq, slot, isWarmup) => {
    const w = workload(seq);
    const contract = pools[slot % pools.length].contracts[w.cc];
    const meta = { run_id, condition, run_index, worker_slot: slot, seq,
                   target_key: isRead ? readTargetKey : w.key, warmup: isWarmup };
    if (isRead) return evaluateRead(contract, w.fn, [readTargetKey], meta);
    return submitSplit(contract, w.fn, w.args, meta);
  };
  ...
  const state_keys_after = await queryStateKeys(opts.contracts.integ);
  if (STAKE_CONDITIONS.has(condition)) {
    stake_after = await queryStakes(opts.gateways, opts.identityIds);
    console.log(`  stake after:  ${JSON.stringify(stake_after)}`);
  }
  ...
  const warmupObserved = recs.filter(r => r.warmup).length;
  const steadyCount = recs.filter(r => r.status === 'COMMITTED' && !r.warmup).length;
  if (steadyCount < 50) {
    console.warn(`  WARNING: steady_count=${steadyCount} (<50). Statistics for this run are weakly supported.`);
  }
  ...
  const heightsAgreeAfter = new Set(ORGS
    .map(o => after.peers && after.peers[o] && after.peers[o].height)).size === 1;
  const hashesAgreeAfter = new Set(ORGS
  ...
  const manifest = {
    ...
    counts: { submitted, committed, errors_by_class: byClass, error_total: errSum,
              ...
              warmup_count_observed: warmupObserved, steady_count: steadyCount },
    suspect_low_steady_count: steadyCount < 50,
    stake_before, stake_after,
    state_keys_before, state_keys_after,
    // A2: set post-phase by resource_coverage.py once the four node-local
    // sampler CSVs are merged. PENDING here means "not yet computed", never
    // "no data" — anything still PENDING at report time is a defect.
    resource_coverage: 'PENDING',
    gateway_deadlines_ms: opts.deadlines,
    clients_n: opts.pools.length,
...
async function main() {
  ...
  // M6: gateway deadlines are CLI-settable and recorded in every manifest.
  // Phase 6 uses commitStatus=30000 so a transaction stranded by an orderer kill
  // cannot block its worker for five minutes and mask the failover gap.
  const deadlines = {
    evaluate:     parseInt(args['deadline-evaluate'] || '15000', 10),
    endorse:      parseInt(args['deadline-endorse']  || '60000', 10),
    submit:       parseInt(args['deadline-submit']   || '60000', 10),
    commitStatus: parseInt(args['deadline-commit']   || '300000', 10),
  };
  console.log(`gateway deadlines (ms): ${JSON.stringify(deadlines)}`);

  // A3: convergence timeout, default 900 s.
  CONVERGE_TIMEOUT_MS = parseInt(args['converge-timeout'] || '900000', 10);
  console.log(`ledger convergence timeout: ${CONVERGE_TIMEOUT_MS} ms`);

  FLUSH_EVERY = Math.max(1, parseInt(args['flush-every'] || '100', 10));
  if (FLUSH_EVERY !== 100) console.log(`jsonl flush interval: ${FLUSH_EVERY} record(s)`);

  // PROBE: N independent grpc.Clients, each its own HTTP/2 connection, each
  // with its own gateway pair. Worker slot i uses pool[i % N].
  const clientsN = Math.max(1, parseInt(args.clients || '1', 10));
  console.log(`grpc clients: ${clientsN}`);
  const pools = [];
  for (let i = 0; i < clientsN; i++) {
    const c  = await newGrpcConnection();
    const ga = await newGatewayForUser(c, ADMIN_USER, deadlines);
    const gu = await newGatewayForUser(c, TEST_USER, deadlines);
    const nu = gu.getNetwork(channelName);
    pools.push({ client: c, gwAdmin: ga, gwUser: gu, contracts: {
      prov:  nu.getContract(chaincodeName, PROV_CC),
      rep:   nu.getContract(chaincodeName, REP_CC),
      integ: nu.getContract(chaincodeName, INTEG_CC),
    }});
  }
  const client   = pools[0].client;
  const gwAdmin  = pools[0].gwAdmin;
  const gwUser   = pools[0].gwUser;
  const contracts = pools[0].contracts;
  const adminRep = gwAdmin.getNetwork(channelName).getContract(chaincodeName, REP_CC);

  // GetStake takes an actorID; normalizeIdentity() returns plain strings
  // unchanged, so the enrolled username is the correct lookup key.
  const identityIds = { [ADMIN_USER]: `${ADMIN_USER}@org1.example.com`,
                        [TEST_USER]:  `${TEST_USER}@org1.example.com` };
  const gateways = { [ADMIN_USER]: gwAdmin, [TEST_USER]: gwUser };
  ...
  // B4: condition D must not silently fail 1,500 times. Create the target, then
  // prove one GetPartTrustReport succeeds against it before any run starts.
  // GetPartTrustReport is registered on IntegrationContract (verified in
  // integration_contract.go:493), not on ProvenanceContract.
  ...
  try {
    ...
    console.error(`FATAL: could not create read target ${readTargetKey}: ${e.message.slice(0, 300)}`);
    process.exit(2);
  }
  try {
    const raw = await contracts.integ.evaluateTransaction('GetPartTrustReport', readTargetKey);
    const rep = JSON.parse(utf8Decoder.decode(raw));
    if (!rep || rep.assetId !== readTargetKey) {
      console.error(`FATAL: GetPartTrustReport returned unexpected payload for ${readTargetKey}`);
      process.exit(2);
    }
    console.log(`setup: read target verified, stage=${rep.currentStage}, ` +
                `history=${(rep.provenanceHistory || []).length}, ` +
                `linkedRatings=${(rep.linkedRatings || []).length}`);
  } catch (e) {
    console.error(`FATAL: GetPartTrustReport failed against ${readTargetKey}: ${e.message.slice(0, 300)}`);
    process.exit(2);
  }

  // DEFECT FIX (2026-08-09): index.json was rebuilt from an empty array on every
  // invocation, so a second invocation against the same --out silently erased
  // the first invocation's entries. This is exactly what happened on 2026-08-04:
  // the surviving phase3 index.json listed only E and F because a later
  // invocation overwrote the A-D entries. Merge instead, keyed on
  // condition+run_index so a genuine re-run replaces its own row and nothing else.
  const indexPath = path.join(outRoot, 'index.json');
  let index = [];
  if (fs.existsSync(indexPath)) {
    try { index = JSON.parse(fs.readFileSync(indexPath, 'utf8')); }
    catch (e) { console.error(`WARN: could not parse existing index.json (${e.message}); starting fresh`); index = []; }
    if (!Array.isArray(index)) index = [];
    if (index.length) console.log(`index.json: merging into ${index.length} existing entr${index.length === 1 ? 'y' : 'ies'}`);
  }
  let firstRun = true;
  ...
  for (const condition of conditions) {
    ...
    for (let r = 1; r <= runs; r++) {
      ...
      const res = await runCondition({
        ...
        gateways, identityIds, deadlines, contracts, pools, readTargetKey,
        cooldownObservedMs, extraManifest,
      });
      const row = { condition, run_index: r, W, total, dir: res.runDir,
                    invariant_holds: res.invariantOk, session_id: sessionId };
      const at = index.findIndex(e => e.condition === condition && e.run_index === r);
      if (at >= 0) index[at] = row; else index.push(row);
      fs.writeFileSync(indexPath, JSON.stringify(index, null, 2));
    }
  }

  for (const p of pools) { p.gwAdmin.close(); p.gwUser.close(); p.client.close(); }
