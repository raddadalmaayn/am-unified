...
| Mechanism | Location |
|---|---|
| child process per logical operation | `run.js:87` — `spawn('node', ['worker.js'])` |
| SIGKILL at pre-registered timings | `run.js:107,120` — `window` and `random` policies |
...
| independent key-based ledger walk | `run.js:143-192` — `verify()` |

*(Line numbers are as of 2026-08-11, after the `--rated-per-trial` addition of
`PHASE_11_REPORT.md` §4.4 shifted `run.js` by ~16 lines.)*
...
The rest of §4 stands, **except its central caveat, which is now resolved — see
§5.1. The binary asymmetry is nominal, not real.**

### 5.1 RESOLUTION — the binary asymmetry is nominal (Phase 10 §8, question 3)

Phase 10 §8 asked whether Design A on June binaries against Design B on current
binaries is acceptable, or whether `prov`/`rep` need rebuilding from current
source. **Neither is needed: the June binaries *are* current source.**

**Neither source repository has changed since the study.** `git log --oneline
--since=2026-06-01` returns **nothing** for either:

| Repo | Last commit | Date | Deployed contract source | mtime |
|---|---|---|---|---|
| `~/AM/am-provenance` | `cbfda1d` | **2026-02-06** | `am_provenance.go` | 2025-07-31 |
| `~/AM/am-reputation` | `0106105` | **2025-11-15** | `chaincode/contract.go` | 2025-10-28 |

Both predate the 2026-06-23 study by months. `am-provenance`'s working tree is
**clean**. `am-reputation` shows 340 dirty paths, of which **336 are under
`chaincode/vendor/`** (vendored `fabric-chaincode-go`,
`fabric-contract-api-go`, `fabric-protos-go-apiv2`). The four non-vendor entries
are a deleted client test, two untracked client-test scripts, and an untracked
`contract.1.go` at the **repository root** dated 2025-11-18 — outside the
`chaincode/` package Go compiles. **No contract logic is modified.**

**The Design A images were built one day before the study**, from that source:

```
prov_ccaas_image:latest  created=2026-06-22T20:15:42-06:00
rep_ccaas_image:latest   created=2026-06-22T20:16:11-06:00
```

**The predecessor assertion never applied to prov or rep.** Commit `9a9db0e`
(2026-08-03, "Assert lifecycle predecessor in the bridge provenance path") lives
in the **`am-unified` repository** and touched three files, all of them unified:

```
 chaincode/unified/helpers.go              | 39 +++++++++++++++++++++++
 chaincode/unified/integration_contract.go | 53 +++++++++++++++++++++----------
 chaincode/unified/provenance_contract.go  | 12 +++----
```

Grepping `am-provenance/*.go` and `am-reputation/chaincode/contract.go` for
`predecessor`, `assertLifecycle` or any lifecycle assertion returns **zero
matches in both**. The assertion is a property of the unified chaincode alone
and was never part of the two-chaincode design in any revision.

**Therefore:** Design A ran code that is byte-for-byte the current state of its
source. There is no pre-/post-assertion gap on the Design A side, because there
was never an assertion to be on either side of. Phase 10 §8 question 3 is
**closed**: `prov`/`rep` do not need rebuilding, and the manuscript does not need
to disclose an asymmetry, because there is none.

**One honest limit on this conclusion.** It rests on git history, file mtimes and
the assertion grep — **not** on rebuilding the images and comparing digests
against the June builds. A rebuild would be conclusive; it was not performed.
Given clean trees and source files months older than the build, a rebuild would
be expected to reproduce, but that is an inference, not a measurement.

**Correction to the description, conclusion unchanged.** §4 calls `prov.tar.gz`
and `rep.tar.gz` "the June binaries". Under CCAAS those files are 428- and
427-byte **connection packages** (`metadata.json` + a `code.tar.gz` holding
`connection.json`) and were regenerated at deploy time on 2026-08-11T03:19:33Z
and 03:21:16Z. The binaries live in the Docker images, which are June-dated as
shown above. The substance — Design A ran June-built code — is confirmed, but
the artifact named was the wrong one.
...
**Archive directory: `results/phase9-archive-20260811T232824Z/`**, containing
`ARCHIVE.md`, the three current reports, and the 15 fragments. The `INDEX.json`
manifest it originally held (26,255 files, each with a sha256, path, size and
mtime) is not published, because it listed absolute operator paths. The raw
run trees are not duplicated here.
...
| ~~prov/rep rebuild from current source~~ | **CLOSED** — June builds are current source; assertion never applied to prov/rep | resolved, §5.1 |
...
| ~~Paper: binary asymmetry disclosure~~ | **CLOSED** — no asymmetry to disclose | resolved, §5.1 |
