...
- `prov` and `rep` images are the **June packages** (`sha256:1639c1d98878…`,
  `sha256:3d85e16391a6…`). Design A is study-era code; Design B is
  post-assertion. **This asymmetry is real and must be stated in the
  manuscript** — it is the same caveat Phase 10 §4 raised, and it is unchanged.
...
- The **binary asymmetry**: Design A ran on June packages, Design B on current
  source (§8). Rebuilding prov/rep from current source remains an open decision
  from Phase 10 §8 question 3.
